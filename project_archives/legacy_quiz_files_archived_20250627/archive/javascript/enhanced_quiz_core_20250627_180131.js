...
// Removed ES6 import - using global API functions instead
...
function initializeMultipleChoice() {
  document.querySelectorAll(".mcq-container, .block-multiple-choice").forEach(container => {
    ...
    // Try to get correct index from container first, then from options grid
    let correctIndex = parseInt(container.dataset.correctIndex, 10);
    if (isNaN(correctIndex)) {
      const optionsGrid = container.querySelector(".mcq-options-grid");
      correctIndex = optionsGrid ? parseInt(optionsGrid.dataset.correctIdx, 10) : 0;
    }
