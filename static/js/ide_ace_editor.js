...
console.log('📦 Loading ACE Editor Integration module...');
...
window.IDEAceEditor = window.IDEAceEditor || {
    ...
    enhanceBasicEditor(element) {
        ...
        if (element.tagName !== 'TEXTAREA') {
            ...
            textarea.value = element.textContent || '# Write your Python code here...\nprint("Hello, Python IDE!")';
            ...
            console.log(`✅ Enhanced basic editor: ${textarea.id}`);
        }
    },
    
    // ===== EDITOR TOOLBAR =====
    initEditorToolbar() {
        console.log('🔧 Initializing editor toolbar...');
        // Toolbar functionality is handled in the template
        // This method is required by the init process
    },
    ...
    // ===== EDITOR SETTINGS =====
    initEditorSettings() {
        console.log('⚙️ Initializing editor settings...');
        // Load saved settings
        this.loadThemePreference();
        
        // Apply default settings to all editors
        Object.values(this.editors).forEach(editorData => {
            if (editorData.editor) {
                this.applyDefaultSettings(editorData.editor);
            }
        });
    },
    
    applyDefaultSettings(editor) {
        if (!editor || !editor.setOptions) return;
    ...
    toggleFullscreen() {
        if (this.currentEditor) {
            const container = this.currentEditor.container;
            if (container) {
                container.style.position = container.style.position === 'fixed' ? 'relative' : 'fixed';
                container.style.top = container.style.position === 'fixed' ? '0' : 'auto';
                container.style.left = container.style.position === 'fixed' ? '0' : 'auto';
                container.style.width = container.style.position === 'fixed' ? '100vw' : 'auto';
                container.style.height = container.style.position === 'fixed' ? '100vh' : 'auto';
                container.style.zIndex = container.style.position === 'fixed' ? '9999' : 'auto';
            }
        }
    },
};
